...
public:
    ...
    /**
     * @brief Interpolates between two configurations respecting joint limits and types.
     */
    std::vector<double> interpolate(const std::vector<double>& start, const std::vector<double>& end, double t) const {
        std::vector<double> result(start.size());

        if (!planning_scene_) {
            ROS_ERROR("ValidityChecker: PlanningScene is null during interpolation.");
            return result;
        }
        ...
        if (!jmg) {
             ROS_ERROR("ValidityChecker: Invalid group name '%s'", group_name_.c_str());
             return result;
        }
        ...
        // Note: This assumes 1-DOF joints (standard for arms), where vector index maps 1-to-1 with active joints.
        for (size_t i = 0; i < start.size() && i < active_joints.size(); ++i) {
            const moveit::core::JointModel* joint = active_joints[i];
            
            // joint->interpolate handles the logic:
            // - If Continuous: Uses shortest angular distance (wrapping)
            // - If Revolute/Prismatic: Uses linear interpolation (end - start) without wrapping
            joint->interpolate(&start[i], &end[i], t, &result[i]);
        }
    ...
    /**
     * @brief Calculates distance respecting joint types (Continuous vs. Revolute).
     */
    double distance(const std::vector<double>& a, const std::vector<double>& b) const {
        if (!planning_scene_) return 0.0;
        ...
        for (size_t i = 0; i < a.size() && i < active_joints.size(); ++i) {
            const moveit::core::JointModel* joint = active_joints[i];
            
            // distance() calculates the correct difference based on joint type
...
bool validateEdgeLinear(const std::vector<double>& start, const std::vector<double>& end, double dist) {
        ...
        for (int i = 1; i < steps; ++i) {
            ...
            // Uncomment the lines below if you want to see every single intermediate state
            // std::stringstream ss_debug;
            // for(auto val : intermediate) ss_debug << val << " ";
...
//     std::ofstream log_file_; // <--- ADDED: File stream for logging

// public:
//     ValidityChecker(planning_scene::PlanningScenePtr scene, double resolution = 0.05) 
//         : planning_scene_(scene), resolution_(resolution) 
//     {
//         // <--- ADDED: Open the file when the checker is initialized
//         // This will create the file in your ~/.ros/ directory by default if running via roslaunch
//         log_file_.open("validated_configurations.txt");
//         if (!log_file_.is_open()) {
//             ROS_WARN("ValidityChecker: Failed to open validated_configurations.txt for writing.");
//         }
//     }

//     // <--- ADDED: Destructor to ensure file is closed properly
//     ~ValidityChecker() {
//         if (log_file_.is_open()) {
//             log_file_.close();
//         }
...
//                 log_file_ << joint_values[i];
//                 if (i < joint_values.size() - 1) {
//                     log_file_ << ", ";
//                 }
//             }
//             log_file_ << "\n"; // Newline for the next configuration
//         }
//         // ------------------------
...
//         if (res.collision) {
//             // ROS_WARN_THROTTLE(1.0, "State is INVALID!");
//             // Commented out throttle warning to reduce console noise during logging
//         }
