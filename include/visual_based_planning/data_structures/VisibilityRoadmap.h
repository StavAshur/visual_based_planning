...
public:
    ...
    void build() {
        if (!vis_oracle_ || !sampler_) {
            ROS_ERROR("[VisibilityRoadmap] Error: Oracle or Sampler not set.");
            return;
        }
        
        // 1. Sample Points
        std::cout << "[VisibilityRoadmap] Sampling " << vi_params_.num_roadmap_samples << " valid points..." << std::endl;
        workspace_samples_.clear();
        sampler_->sampleValidPoints(vi_params_.num_roadmap_samples, workspace_samples_, vi_params_.face_samples);
        
        // 2. Precompute Visibility between Samples (New Definition of Vis(p))
        std::cout << "[VisibilityRoadmap] Computing sample-to-sample visibility..." << std::endl;
        computeSampleVisibility();

        // 3. Cluster based on VI
        std::cout << "[VisibilityRoadmap] Clustering..." << std::endl;
        performClustering();

        // 4. Build Cluster Visibility Graph
        std::cout << "[VisibilityRoadmap] Building Cluster Visibility Graph for " << clusters_.size() << " clusters..." << std::endl;
...
/**
    ...
    void computeSampleVisibilityGPU() {
        std::cout << "[VisibilityRoadmap] GPU Acceleration: Computing " 
    ...
    void performClustering() {
        ...
        ROS_INFO("[VisibilityRoadmap] Clustering Complete. Total Clusters: %lu", clusters_.size());
        computeAndPrintStats("All - Radius", all_rads);
        computeAndPrintStats("All - Size  ", all_sizes);
        
        ROS_INFO("[VisibilityRoadmap] Small Clusters (<10 pts): %lu", small_sizes.size());
        ...
        if (outfile.is_open()) {
            ...
            outfile.close();
            std::cout << "[VisibilityRoadmap] Saved clustered points to clustered_points.txt" << std::endl;
        } else {
            std::cerr << "[VisibilityRoadmap] Error: Could not open clustered_points.txt for writing." << std::endl;
        }
    }
    ...
    void buildClusterVisibilityGraph() {
        ...
        Eigen::Vector3d goal_min(-5.0, -5.0, 0.0);
        Eigen::Vector3d goal_max(5.0, 5.0, 1.4);
        ...
        ROS_WARN("[VisibilityRoadmap] Graph built. Total edges: %d",total_edges);
    }
